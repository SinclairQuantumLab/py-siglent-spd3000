...
In the connection cell, replace `<TYPE>` in `spd.ConnectionType.<TYPE>` with an enum member such as `SOCKET` or `VISA`, replace `"<IDENTIFIER>"`, and uncomment only the connection-specific argument described for VISA or an authenticated gateway.
