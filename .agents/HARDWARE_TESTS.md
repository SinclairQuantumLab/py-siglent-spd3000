...
- [ ] `SET-03`: Set CH1 to a low model-aligned voltage and current limit inside `with psu.batch, psu.verify:`, require one non-interleaved write/readback batch, and require exact values at the model resolution.
...
- [ ] `SET-06`: Restore the initial CH1 and CH2 voltage and current settings in a write-only `psu.batch`, then query them back after the safety-critical restoration batch has completed.
